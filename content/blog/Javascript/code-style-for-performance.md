...
우선 `for-in` 구문이 테스트 결과 가장 느리다. 나머지 반복문들은 별 차이가 없다. 왜 느릴까? 그 이유는 다른 반복문과 달리 주어진 배열을 순차적으로 탐색하는게 아니라 배열이 아닌 객체로 취급하여 반복 시점마다 객체의 모든 속성을 무작위로 탐색한다. 권장사항으로는 객체의 속성을 탐색하는데만 사용하길...
